...
badRequestBody =
  "<!DOCTYPE html><html><head><title>Bad Request</title></head><body><h1>Bad request.</h1>It could be my fault. When I grow up I want to be a full-featured HTTP server.</body></html>"
notFoundBody =
  "<!DOCTYPE html><html><head><title>Not Found</title></head><body><h1>404!</h1>File not found!</body></html>"
...
censor f (WriterT mwa) = WriterT $ fmap (\(a, w) -> (a, f w)) mwa
...
instance Functor m => Functor (WriterT w m) where
  ...
  fmap f (WriterT mwa) = WriterT $ fmap (\(a, w) -> (f a, w)) mwa
...
instance (Monoid w, Applicative m) => Applicative (WriterT w m) where
  ...
  (<*>)
    ...
    -> WriterT w m a
    -> WriterT w m b
  (WriterT mwf) <*> (WriterT mwa) =
    WriterT $ liftA2 (\(f, w) (a, w') -> (f a, w <> w')) mwf mwa
...
instance Monoid w => MonadTrans (WriterT w) where
  ...
  lift monad = WriterT $ (\a -> (a, mempty)) <$> monad
...
main = Socket.withSocketsDo $ do -- withSocketsDo is supposedly not needed anymore on most systems.
  ...
  args <- getArgs
  let portNumber = if null args then defaultPortNumber else read . head $ args
...
handleConn connHandle peerAddress = do
 ...
 where
  ...
  prependPeerName logLines = map (prefix ++) logLines
    where prefix = "[" ++ show peerAddress ++ "] "
...
readAndRespond connHandle = do
  ...
  case maybeReqLines of
    ...
    Just reqLines -> case maybeParseRequestLines reqLines of
...
maybeParseRequestLines (firstLine : _) = case words firstLine of
...
maybeReadOneFile (path : rest) = (Just <$> B.readFile path) `catch` tryNext
 where
...
requestPathToFilePaths path =
  let parts = htmlFolder : splitRegex (mkRegex "/|\\\\|\\.\\.") path
...
serializeResponse (HTTPResponse statusCode body) = B.concat
  [ "HTTP/1.1 "
  , statusCode
  , crlf
  ...
  , lengthStr
  , crlf
  , "Server: Lab 7 Silly Haskell Server 1.0"
  , crlf
  , "Connection: close"
  , crlf
  , ""
