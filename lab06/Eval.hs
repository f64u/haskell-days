...
evalMathExp bindings exp =
  ...
  in
    case exp of
      ...
      Div   e1 e2 -> if recurse e2 == Right 0
        then Left "division by zero"
        else binOp quot e1 e2
      Pow e1 e2 -> case recurse e2 of
        Right pow ->
          if pow < 0 then Left "negative exponent" else binOp (^) e1 e2
        Left err -> Left err
      Let names mathExps mainExp | length names == length mathExps -> do
        newbindings <-
          zip names <$> (allRight . map (evalMathExp bindings) $ mathExps)
        evalMathExp (bindings ++ newbindings) mainExp
